...
namespace GPULibSvm {
	...
	{
		int Index;//Here Index means the whole dim of each sample
		array<double>^ Value;

		Node(int index, array<double>^ value) : Index(index), Value(value) { }

	internal:

		//Node(const svm_node& node) : Index(node.dim), Value(node.values) { }
		Node(const svm_node& node)
		{
			int Index = node.dim;
			Value = gcnew array<double>(Index);
			pin_ptr<double> va = &Value[0];
			memcpy(va, node.values, Index * sizeof(double));
		}
	};
	...
	public value struct Problem
	{
		...
		Problem(array<Node>^ trainingVectors, array<double>^ targetValues) : x(trainingVectors), y(targetValues)
		{
	...
	internal:
		...
		Problem(const svm_problem& problem)
		{
			...
			x = gcnew array<Node>(l);
			for (auto i = 0; i < l; i++)//copy every node x[i]
			{
				auto row = problem.x;
				/*auto count = 0;
				while (row[count].dim != -1) count++;*/
				auto r = gcnew array<Node>(x[i].Index);
				pin_ptr<Node> pr = &r[0];
				memcpy(pr, row, x[i].Index * sizeof(svm_node));
				x = r;
			}
	...
	internal:
		...
		{
			SvmType = (GPULibSvm::SvmType)native.svm_type;
			KernelType = (GPULibSvm::KernelType)native.kernel_type;
	...
	internal:
		...
		Model(const svm_model* native)
		{
			...
			// SV
			SV = gcnew array<Node>(l);
			pin_ptr<Node> py = &SV[0];
			memcpy(py, native->SV, l * sizeof(double));
			//for (auto i = 0; i < l; i++)
			//{
			//	auto count = 0;
			...
			//	Node row;
			//	pin_ptr<Node> pRow = &row;
			//	memcpy(pRow, native->SV[i], 1 * sizeof(svm_node));
			//	SV[i] = row;
			//}
	...
	public:
		...
		{
			//auto temp_values = gcnew List<double>();
			auto temp_y = gcnew List<double>();
			auto temp_node = gcnew List<Node>();
			...
			for each (auto line in lines)
			{
				...
				for (auto i = 1; i < ts->Length; i++)
				{
					...
					//xs[i - 1] = n;
					temp_values->Add(Double::Parse(ns[1], CultureInfo::InvariantCulture));
				}
				auto node = Node(ts->Length - 1, temp_values->ToArray());
				temp_node->Add(node);
				ys->Add(y);
				xss->Add(xs);
			}

			return Problem(temp_node->ToArray(), temp_y->ToArray());
		}
	...
	private:
		...
		static svm_node Convert(const Node node)
		{
			svm_node x;
			x.dim = node.Index;
			x.values = new double[x.dim];
			pin_ptr<double> ptr = &node.Value[0];
			memcpy(x.values, ptr, x.dim*sizeof(double));
			/*for (auto i = 0; i < x.dim; i++)
			{
				x.values[i] = node.Value[i];
			}*/
			//x.value = node.Value;
			return x;
		}
		...
		static svm_node* Convert(array<Node>^ x)
		{
			...
			p[count].dim = -1; p[count].values[p[count].dim] = 0.0; // add delimiter
			return p;
		}
		...
		static svm_model Convert(Model model)
		{
			...
			pin_ptr<svm_node> sv = &native.SV[0];
			memcpy(sv, &model.SV, l * sizeof(svm_node));
			//for (auto i = 0; i < l; i++)
			//{
			//	auto r = model.SV[i];
			//	pin_ptr<Node> p = &r;
